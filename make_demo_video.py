...
def main():
    ...
    for index, row in enumerate(rows):
        ...
        canvas = Image.new("RGB", (1280, 720), "#101522")
        image.thumbnail((900, 650))
        canvas.paste(image, (30 + (900 - image.width) // 2, 35))
        draw = ImageDraw.Draw(canvas)
        image_x = 30 + (900 - image.width) // 2
        image_y = 35
        ...
        panel_x = 965
        draw.text((panel_x, 50), "MULTIMODAL JEV", fill="#ffffff", font=font)
        draw.text((panel_x, 80), "Question: Which bird species?", fill="#b9c4d6", font=font)
        draw.text((panel_x, 120), f"Prediction: {row['options'][predicted_index].replace('_', ' ')}", fill="#ffffff", font=font)
        draw.text((panel_x, 155), f"Ground truth: {row['class_name'].split('.', 1)[-1].replace('_', ' ')}", fill="#46e37b", font=font)
        draw.text((panel_x, 205), "Option probabilities", fill="#ffffff", font=font)
        for choice_index, (option, probability) in enumerate(zip(row["options"], probabilities)):
            y = 240 + choice_index * 65
            label = option.replace('_', ' ')
            draw.text((panel_x, y), f"{choice_index}: {label[:22]}", fill="#dce4f2", font=font)
            draw.rectangle((panel_x, y + 20, panel_x + 250, y + 35), fill="#26344d")
            draw.rectangle((panel_x, y + 20, panel_x + int(250 * probability), y + 35), fill="#5aa9ff" if choice_index != predicted_index else "#ffbd59")
            draw.text((panel_x + 260, y + 18), f"{probability:.3f}", fill="#ffffff", font=font)
        draw.text((panel_x, 530), "Red: predicted box", fill="#ff4d5e", font=font)
        draw.text((panel_x, 555), "Green: ground-truth box", fill="#46e37b", font=font)
        draw.text((panel_x, 620), f"CUB test image {index + 1}/{len(rows)}", fill="#7f8da6", font=font)
